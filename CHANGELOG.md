...
- **SearchResultRow descriptive zone now reads as one VoiceOver stop** — `Result <rank>, in library, <title>, by <author>` instead of four separate stops for rank, status chip, title, and author. A 25-result list previously cost 100+ swipes before reaching the `+ ADD TO LIBRARY` and `→ WEBSITE` action keys; now it's 25 stops + 2 actions per row. Action keys keep their own a11y elements. Mirrors the combine pass applied to LibraryDirectoryRow and PodcastEpisodeTunerRow.
