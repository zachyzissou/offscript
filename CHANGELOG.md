...
- **HeroTunerCard metadata + EpisodeDetail timeRemaining now read naturally for VoiceOver** — adds `voiceOverMetadata` on the Hero card (matching #267 / #268) and `spokenTimeRemaining` ("fifteen minutes left") on EpisodeDetail's progress strip, so VO no longer walks the visible mono "MAY 1, 2026 · 1H 5M" / "15M LEFT" glyph-by-glyph.
