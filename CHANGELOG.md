...
- **PodcastDetail episode row a11y label now uses VO-friendly metadata** — the rich VO label introduced in #265 was interpolating the visible mono `metadata` ("E12 · MAY 1, 2026 · 1H 5M") which VoiceOver speaks the "·" / abbreviations literally. The new `voiceOverMetadata` variant drops uppercasing and the "·" separator, expanding "S2 E5" to "Season 2 Episode 5" so the readout pronounces sensibly. Visible mono `metadata` unchanged. Same pattern as the rail-card fix from PR #267.
