...
- **PodcastDetail episode rows' NavigationLink now reads as one VoiceOver stop** — `Open <title>, <metadata>` instead of three separate stops for rank, title, and metadata. The metadata readout already includes the episode's rank/duration/age glyphs, so swallowing the bare rank avoids a duplicate. The sibling Play / Queue / More action keys keep their own a11y elements so the per-row count is one navigation stop + up to three action stops, down from ~6+. Mirrors the combine pass on EpisodeDetail FROM CHANNEL chip (#249) and SearchResultRow (#261).
