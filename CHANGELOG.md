...
- **Background feed refresh, deep-link episode lookup, `deleteAllDownloads`, `resumeQueuedDownloadsIfNeeded`, and `reconcilePersistedDownloads` now log the underlying SwiftData fetch error** instead of dropping it via bare `try?` — so a failed background refresh, a broken episode deep link, or a download lifecycle hiccup leaves a real OSLog `.error` entry under `BackgroundFeedRefresh` / `DeepLink` / `DownloadService` instead of disappearing silently.
