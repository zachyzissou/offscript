...
- **Library SHOWS · DIRECTORY rows now expose rich VoiceOver labels** — `Open channel <N>, <title>, by <author>, <X> in progress, <Y> unplayed, sync failed` instead of the prior bare `Open <title> channel`. The parent NavigationLink's `.accessibilityLabel` was clobbering every child, so VO users could not hear the channel number, in-progress / unplayed counts, or the `● SYNC FAILED` chip that sighted users see next to the title. Empty-author and zero-in-progress clauses are dropped so the label stays clean for healthy rows.
