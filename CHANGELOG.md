...
- **Background transcription's downloaded-episode candidate scan and Apple ID embedded-provisioning-profile entitlement extraction now log the underlying error** instead of dropping it via bare `try?` — so a failed transcription scan or a malformed embedded `embedded.mobileprovision` leaves a real OSLog entry under `BackgroundTranscription` (`.error`) or the new `AppleIdentity` category (`.warning`) instead of disappearing silently.
