...
- **Discovery rail cards now bottom-align their `+ TUNE` action key uniformly** by reserving a fixed 2-line slot for the title and absorbing reason-tag / SOURCE-trace height variance with a `Spacer()` before the action. Stops the rail from staircasing when sibling card titles wrap to different line counts (#180).
