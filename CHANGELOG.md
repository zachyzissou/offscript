...
- **Queue rows are now tappable to open Episode Detail** — rank + artwork + title/metadata wrapped in a NavigationLink while the play / move / remove action buttons stay siblings. Matches the PodcastEpisodeTunerRow pattern; gives queued-item access to chapters, transcripts, and feedback without leaving the Queue tab.
